...
async function readData(): Promise<DataSchema> {
  try {
    ...
    return {
      ...
      declinedKeys: Array.isArray(parsed.declinedKeys) ? parsed.declinedKeys : [],
    };
  } catch {
    // Jika file tidak ada atau gagal parsing, kembalikan schema kosong
    return { arrivedKeys: [], undockedKeys: [], declinedKeys: [] };
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "check") {
      ...
      // Menandai kapal batal datang (Decline)
      declinedSet.add(shipKey);
      arrivedSet.delete(shipKey);
      undockedSet.delete(shipKey);
    }
    ...
    const updatedData: DataSchema = {
      ...
      declinedKeys: Array.from(declinedSet),
    };
