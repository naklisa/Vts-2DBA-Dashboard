...
interface ArrivalStatusFilterProps {
  statusFilter: "semua" | "tiba" | "batal";
  onChange: (status: "semua" | "tiba" | "batal") => void;
}

export default function ArrivalStatusFilter({ statusFilter, onChange }: ArrivalStatusFilterProps) {
  ...
  const handleSelect = (val: "semua" | "tiba" | "batal") => {
    onChange(val);
    setIsOpen(false);
  };

  const getLabel = () => {
    if (statusFilter === "tiba") return "Telah Tiba";
    if (statusFilter === "batal") return "Batal / Tidak Jadi Sandar";
    return "Semua Status";
  };
    ...
    <div ref={containerRef} className="flex flex-col gap-1.5 min-w-[220px] relative z-30">
      ...
      {isOpen && (
        <div className="absolute top-[72px] left-0 z-50 w-full rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/95 text-zinc-800 dark:text-zinc-100 p-2 shadow-2xl backdrop-blur-md animate-in fade-in slide-in-from-top-2 duration-150">
          ...
          <button
            type="button"
            onClick={() => handleSelect("tiba")}
            className={`w-full text-left px-4 py-2.5 mt-1 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer ${statusFilter === "tiba"
                ? "bg-cyan-500 text-white dark:text-zinc-950 font-extrabold shadow-md shadow-cyan-500/10"
                : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-black dark:hover:text-white"
              }`}
          >
          ...
          <button
            ...
            className={`w-full text-left px-4 py-2.5 mt-1 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer ${statusFilter === "batal"
                ? "bg-cyan-500 text-white dark:text-zinc-950 font-extrabold shadow-md shadow-cyan-500/10"
                : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-black dark:hover:text-white"
              }`}
          >
            Batal / Tidak Jadi Sandar
          </button>
        </div>
      )}
    </div>
